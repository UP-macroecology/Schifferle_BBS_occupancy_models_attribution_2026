...
# quadratic effects of bioclim covariates, lasso prior, p intercept only
# run for multiple species in parallel
...
model_file <- "dynoccmod_cl_quadr_lasso_GoF.txt"
  ...
  # Iterate over sites:
	for (s in 1:nsites){
		...
		logit(psi1[s]) <- alpha_psi + beta_psi[1]*bio2_3yrs[s] + beta_psi[2]*bio3_3yrs[s] + beta_psi[3]*bio5_3yrs[s] +
		                  ...
		                  beta_psi[7]*pow(bio2_3yrs[s],2) + beta_psi[8]*pow(bio3_3yrs[s],2) +
		                  beta_psi[9]*pow(bio5_3yrs[s],2) + beta_psi[10]*pow(bio6_3yrs[s],2) +
		                  beta_psi[11]*pow(bio13_3yrs[s],2) + beta_psi[12]*pow(bio14_3yrs[s],2)
    ...
    # State transitions
      ...
      # Set formula for Epsilon (Extinction)
			logit(eps[s, t-1]) <- alpha_eps + beta_eps[1]*bio2[s,t] + beta_eps[2]*bio3[s,t] + 
			                      ...
			                      beta_eps[6]*bio14[s,t] + 
		                        beta_eps[7]*pow(bio2[s,t],2) + beta_eps[8]*pow(bio3[s,t],2) +
		                        beta_eps[9]*pow(bio5[s,t],2) + beta_eps[10]*pow(bio6[s,t],2) +
		                        beta_eps[11]*pow(bio13[s,t],2) + beta_eps[12]*pow(bio14[s,t],2)
			...
			logit(gamma[s, t-1]) <- alpha_gamma + beta_gamma[1]*bio2[s,t] + beta_gamma[2]*bio3[s,t] + 
			                        ...
			                        beta_gamma[6]*bio14[s,t] +
		                          beta_gamma[7]*pow(bio2[s,t],2) + beta_gamma[8]*pow(bio3[s,t],2) +
		                          beta_gamma[9]*pow(bio5[s,t],2) + beta_gamma[10]*pow(bio6[s,t],2) +
		                          beta_gamma[11]*pow(bio13[s,t],2) + beta_gamma[12]*pow(bio14[s,t],2)
...
foreach(spec = testspecs,
        ...
        .verbose = TRUE) %dopar% {
          ...
          sink(paste0("out_cl_quadr_lasso_", gsub(" ", "_", spec),"_GoF.txt")) # write console output here
          sink(type = "message")
          ...
          out <- autojags(data = bdata, inits = inits, parameters.to.save = params, 
                          ...
                          n.cores = 3,
                          Rhat.limit = 1.1,
                          max.iter = 50000) # Rushing: 50000, Mingjian: 20000
          
          save(out, file = file.path(dir, "data", paste0(gsub(" ", "_", spec), "_cl_quadr_lasso_GoF.RData")))
...
# species with similar distribution as house finch:
# "American Robin", "American Crow", "American Goldfinch", "Barn Swallow", "Brown-headed Cowbird", "Chipping Sparrow", 
# "Cliff Swallow", "Common Yellowthroat", "European Starling", "House Wren", "House Sparrow", "Mourning Dove",
# "Northern Flicker", "Northern Rough-winged Swallow", "Red-winged Blackbird", "Red-tailed Hawk", "Rock Pigeon",
# "Turkey Vulture"

# species in the Eastern US:
# "Acadian Flycatcher", "Baltimore Oriole", "Black−and−white Warbler", "Black Vulture", "Blue Jay",
# "Broad−winged Hawk", "Carolina Chickadee", "Brown Thrasher", "Carolina Wren", "Chimney Swift",
# "Eastern Bluebird", "Eastern Towhee", "Eastern Wood−Pewee", "Eastern Kingbird", "Eastern Meadowlark",
# "Eastern Phoebe", "Field Sparrow", "Great Crested Flycatcher", "Indigo Bunting",
# "Kentucky Warbler", "Louisiana Waterthrush", "Northern Cardinal", "Northern Parula", "Northern Bobwhite",
# "Purple Martin", "Orchard Oriole", "Red−bellied Woodpecker", "Red−eyed Vireo", "Ruby−throated Hummingbird",
# "Scarlet Tanager", "Tufted Titmouse", "White−eyed Vireo", "Wood Thrush", "Yellow−billed Cuckoo",
# "Yellow−throated Vireo"

# species in the Western US:
# "Black−billed Magpie", "Brewer's Blackbird", "Black−headed Grosbeak", "Bullock's Oriole", 
# "Rock Wren", "Say's Phoebe", "Spotted Towhee", "Steller's Jay", "Violet−green Swallow", 
# "Western Meadowlark", "Western Tanager", "Western Wood−Pewee", "Western Kingbird"

# species in the North:
# "Alder Flycatcher", "American Redstart", "Black−billed Cuckoo", "Blue−headed Vireo", "Bobolink", 
# "Black−capped Chickadee", "Black−throated Green Warbler", "Cedar Waxwing", "Brown Creeper",
# "Chestnut−sided Warbler", "Clay−colored Sparrow", "Evening Grosbeak", "Golden−crowned Kinglet",
# "Gray Partridge", "Hermit Thrush", "Least Flycatcher", "Mourning Warbler", "Nashville Warbler",
# "Northern Waterthrush", "Purple Finch", "Ruffed Grouse", "Rose−breasted Grosbeak", "Sedge Wren", "Swamp Sparrow",
# "Veery", "Winter Wren", "Yellow−bellied Sapsucker"

# species in the South:
# "Ash−throated Flycatcher", "Black−chinned Hummingbird", "Greater Roadrunner", "Lesser Goldfinch", 
# "Mississippi Kite", "Painted Bunting", "Scissor−tailed Flycatcher"
