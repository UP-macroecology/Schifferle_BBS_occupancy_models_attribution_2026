...
plot_df_lodes <- to_lodes_form(plot_df,
                               axes = c(1:2),
                               id = trend_change_clim) %>%
  mutate(x = factor(x, levels = c("cfclim", "fact"))) %>%
  # same order as in plot_df: order in df determines rendering order
  mutate(trend_change_clim2 = rep(plot_df$trend_change_clim, 2))
...
plot_df_lodes <- to_lodes_form(plot_df, axes = c(1:2), id = trend_change_lu ) %>% 
  mutate(x = factor(x, levels = c("cflu", "fact"))) %>% 
  # same order as in plot_df:
  mutate(trend_change_lu2 = rep(plot_df$trend_change_lu, 2))
...
plot_df_lodes <- to_lodes_form(plot_df,
  ...
  # same order as in plot_df:
  mutate(trend_change_climlu2 = rep(plot_df$trend_change_climlu, 2))
