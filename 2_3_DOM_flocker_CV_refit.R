...
# refit models with more warmup and iterations for species and folds, for which MCMC failed in first round:

# (first round: 2_1_2_DOM_flovker_CV.R)
...
# load folds and species for which MCMC with 1000 + 1000 iterations failed (spec_folds_MCMC_fail; output of 2_2_DOM_check_flocker_CV.R)
load(file.path("data", paste0("MCMC_check_CV_buffer_",  buffer_km, "_MCMC_fail.RData")))
spec_folds_MCMC_fail
...
species_set <- species_set[-c(5, 6, 10, 11, 18, 22, 24, 28, 33)]
...
ncores <- 60 # 5 fold * 4 chains * species
...
foreach(spec = species_set,
        ...
        .verbose = TRUE) %:% 
          ...
          foreach(fold = spec_folds_MCMC_fail[[which(names(spec_folds_MCMC_fail) == spec)]], # only those folds for which MCMC failed
                  ...
                  .verbose = TRUE) %dopar% {
                    ...
                    log_file_spec_fold <- file(file.path("CV", "round_2_2000_2000", paste0(spec, "_CV_fitting_fold", fold, ".txt")), open = "wt") # write console output here
                    ...
                    out <- flock(
                      ...
                      chains = 4,
                      warmup = 2000,
                      iter = 2000 + 2000 # 250 + 1000 # including warmup
                    )
                    
                    print(out)
                    
                    # save fitted model for each fold:
                    save(out, file = file.path(dir, "results", "CV_round2_2000_2000", paste0("out_", spec, "_CV_fold", fold, ".RData")))
                    ...
                    save(res_list, file = file.path(dir, "results", "CV_round2_2000_2000", paste0("test_preds_", spec, "_CV_fold", fold, ".RData")))
