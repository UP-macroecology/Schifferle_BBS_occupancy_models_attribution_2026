...
model_file <- "dynoccmod_cl_alt3_quadr_uninf_GoF.txt"
...
## Declare priors ------------------------------------------------------------------------------
  ...
  for (i in 1:10) { 
	  beta_psi[i] ~ dnorm(0, 0.5) # variance = 2, other than Saoirses code
	}
...
foreach(spec = testspecs,
        ...
        .verbose = TRUE) %dopar% {
          ...
          sink(paste0("out_cl_alt3_quadr_uninf_", gsub(" ", "_", spec),"_GoF.txt")) # write console output here
          sink(type = "message")
          ...
          save(out, file = file.path(dir, "data", paste0(gsub(" ", "_", spec), "_cl_alt3_quadr_uninf_GoF.RData")))
