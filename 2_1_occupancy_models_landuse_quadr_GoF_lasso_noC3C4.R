...
sink("output_landuse_noC3C4_quadr_lasso_House_Finch.txt") # write console output here
...
model_file <- "dynoccmod_lu_noC3C4_quadr_lasso.txt"
  ...
  # Iterate over sites:
	for (s in 1:nsites){
		...
		logit(psi1[s]) <- alpha_psi + 
		                  beta_psi[1]*sum_annual_crops_3yrs[s] +
		                  beta_psi[2]*primn_3yrs[s] + beta_psi[3]*secdn_3yrs[s] + beta_psi[4]*pastr_3yrs[s] +
		                  beta_psi[5]*urban_3yrs[s] +
		                  beta_psi[6]*pow(sum_annual_crops_3yrs[s],2) + beta_psi[7]*pow(primn_3yrs[s],2) +
		                  beta_psi[8]*pow(secdn_3yrs[s],2) + beta_psi[9]*pow(pastr_3yrs[s],2) +
		                  beta_psi[10]*pow(urban_3yrs[s],2)
    ...
    # State transitions
      ...
      # Set formula for Epsilon (Extinction)
			logit(eps[s, t-1]) <- alpha_eps + beta_eps[1]*sum_annual_crops[s,t] +
			                      beta_eps[2]*primn[s,t] +
			                      beta_eps[3]*secdn[s,t] + beta_eps[4]*pastr[s,t] + beta_eps[5]*urban[s,t] +
		                        beta_eps[6]*pow(sum_annual_crops[s,t],2) +
		                        beta_eps[7]*pow(primn[s,t],2) +
		                        beta_eps[8]*pow(secdn[s,t],2) + beta_eps[9]*pow(pastr[s,t],2) +
		                        beta_eps[10]*pow(urban[s,t],2)
			
			# Set formula for Gamma (Colonisation)
			logit(gamma[s, t-1]) <- alpha_gamma + beta_gamma[1]*sum_annual_crops[s,t] +
			                        beta_gamma[2]*primn[s,t] +
			                        beta_gamma[3]*secdn[s,t] + beta_gamma[4]*pastr[s,t] + beta_gamma[5]*urban[s,t] +
		                          beta_gamma[6]*pow(sum_annual_crops[s,t],2) +
		                          beta_gamma[7]*pow(primn[s,t],2) +
		                          beta_gamma[8]*pow(secdn[s,t],2) + beta_gamma[9]*pow(pastr[s,t],2) +
		                          beta_gamma[10]*pow(urban[s,t],2)
...
save(out, file = file.path(cluster_import, "data", paste0(gsub(" ", "_", spec), "_lu_noC3C4_quadr_lasso.RData"))) # 2 GB
