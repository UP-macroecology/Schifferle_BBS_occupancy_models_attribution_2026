# Extract values of selected climatic and land-use variables at route centroids for occupancy models:

# 1) extract values of clim. and land use variables at each route for each year
# 2) extract clim. and land use values summarising 3 years before the focal period (as predictors of initial occupancy)
...
# 1) env. data for each route-year combination: -----------
...
# reduce to selected variables:
lu_3yrs_sp_sel <- lu_3yrs_sp[[selvar_final[!grepl(pattern = "bio|pr_|mean", x = selvar_final)]]] # xx
