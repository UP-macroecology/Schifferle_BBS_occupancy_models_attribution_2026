...
# with flocker, test non-horseshoe priors
...
# intercept:
prior_settings_intercept <- c("logistic(0,1)",
                              "logistic(0,2)",
                              "logistic(0,3)")
# parameters:
prior_settings_b <- c("normal(0,2)",
                      "normal(0,1)",
                      "normal(0,3)",
                      "horseshoe(df = 3, par_ratio = 0.3, df_global = 1, scale_slab = 2, df_slab = 4)",
                      "horseshoe(df = 1, par_ratio = 0.3, df_global = 1, scale_slab = 2, df_slab = 4)")
prior_settings <- expand.grid(prior_settings_intercept, prior_settings_b, stringsAsFactors = FALSE)
names(prior_settings) <- c("Intercept", "b")
prior_settings <- prior_settings[ c(2, 3, 4, 7, 11), ]

#n_warmup <- c(250, 500, 750, 1250, 1500)

# register cores for parallel computation:
ncores <- 20 # par set * 4 chains? 
cl <- makeCluster(ncores, setup_timeout = 0.5)
registerDoParallel(cl)

foreach(prior_set = 1:nrow(prior_settings),
        ...
        .verbose = TRUE) %dopar% {
          ...
          sink(paste0("out_cl_lu_p_fl_AG_", prior_settings$Intercept[prior_set], "_",
                      prior_settings$b[prior_set], ".txt")) # write console output here
          ...
          out <- flock(
            ...
            f_ex = ~ bio1 + bio2 + bio3 + pr_spring + pr_summer + pr_autumn + pr_winter + bio15 +
              ...
              I(sum_annual_crops^2) + I(secdn^2) + I(pastr^2) + I(urban^2) + I(primn^2),
            flocker_data = fd,
            prior = c(brms::set_prior(prior_settings$Intercept[prior_set], class = "Intercept") + # flat on probability scale (https://cran.r-project.org/web/packages/flocker/vignettes/flocker_tutorial.html)
                      brms::set_prior(prior_settings$Intercept[prior_set], class = "Intercept", dpar = "occ"),
                      brms::set_prior(prior_settings$Intercept[prior_set], class = "Intercept", dpar = "colo"),
                      brms::set_prior(prior_settings$Intercept[prior_set], class = "Intercept", dpar = "ex"),
                      brms::set_prior(prior_settings$b[prior_set], class = "b"),
                      brms::set_prior(prior_settings$b[prior_set], dpar = "occ", class = "b"),
                      brms::set_prior(prior_settings$b[prior_set], dpar = "colo", class = "b"),
                      brms::set_prior(prior_settings$b[prior_set], dpar = "ex", class = "b")),
            multiseason = "colex",
            multi_init = "explicit",
            backend = "cmdstanr",
            cores = 4,
          ...
          save(out, file = file.path(dir, "data", paste0("AG_cl_lu_p_flock_", prior_settings$Intercept[prior_set], "_",
                                                         prior_settings$b[prior_set], ".RData")))
          ...
          res_list <- list("fitted" = fitted_occ_col_ex,
                           ...
                           "loo_cv" = loo_cv)
          save(res_list, file = file.path(dir, "data", paste0("AG_cl_lu_p_flocker_", prior_settings$Intercept[prior_set], "_",
                                                              prior_settings$b[prior_set], "_fitted_preds_loo.RData")))
