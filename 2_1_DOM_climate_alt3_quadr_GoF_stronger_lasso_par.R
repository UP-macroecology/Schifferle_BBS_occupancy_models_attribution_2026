...
model_file <- "dynoccmod_cl_alt3_quadr_stronger_lasso_GoF.txt"
...
## Declare priors ------------------------------------------------------------------------------
  ...
  for (i in 1:10) { 
	...
	lambda_psi ~ dt(0,1.566,7.763)T(0,)
  ...
  tau_eps <- 1/lambda_eps 
	lambda_eps ~ dt(0,1.566,7.763)T(0,)
  ...
  tau_gamma <- 1/lambda_gamma 
	lambda_gamma ~ dt(0,1.566,7.763)T(0,)
  ...
  # Iterate over sites:
	for (s in 1:nsites){
		...
		logit(psi1[s]) <- alpha_psi + beta_psi[1]*bio1_3yrs[s] + beta_psi[2]*bio2_3yrs[s] + beta_psi[3]*bio3_3yrs[s] +
		                  beta_psi[4]*bio12_3yrs[s] + beta_psi[5]*bio15_3yrs[s] +
		                  beta_psi[6]*pow(bio1_3yrs[s],2) + beta_psi[7]*pow(bio2_3yrs[s],2) +
		                  beta_psi[8]*pow(bio3_3yrs[s],2) + beta_psi[9]*pow(bio12_3yrs[s],2) +
		                  beta_psi[10]*pow(bio15_3yrs[s],2)
    ...
    # State transitions
      ...
      # Set formula for Epsilon (Extinction)
			logit(eps[s, t-1]) <- alpha_eps + beta_eps[1]*bio1[s,t] + beta_eps[2]*bio2[s,t] + 
			                      beta_eps[3]*bio3[s,t] + beta_eps[4]*bio12[s,t] + beta_eps[5]*bio15[s,t] +
		                        beta_eps[6]*pow(bio1[s,t],2) + beta_eps[7]*pow(bio2[s,t],2) +
		                        beta_eps[8]*pow(bio3[s,t],2) + beta_eps[9]*pow(bio12[s,t],2) +
		                        beta_eps[10]*pow(bio15[s,t],2)
			
			# Set formula for Gamma (Colonisation)
			logit(gamma[s, t-1]) <- alpha_gamma + beta_gamma[1]*bio1[s,t] + beta_gamma[2]*bio2[s,t] + 
			                        beta_gamma[3]*bio3[s,t] + beta_gamma[4]*bio12[s,t] + beta_gamma[5]*bio15[s,t] +
		                          beta_gamma[6]*pow(bio1[s,t],2) + beta_gamma[7]*pow(bio2[s,t],2) +
		                          beta_gamma[8]*pow(bio3[s,t],2) + beta_gamma[9]*pow(bio12[s,t],2) +
		                          beta_gamma[10]*pow(bio15[s,t],2)
...
foreach(spec = testspecs,
        ...
        .verbose = TRUE) %dopar% {
          ...
          sink(paste0("out_cl_alt3_quadr_stronger_lasso_", gsub(" ", "_", spec),"_GoF.txt")) # write console output here
          sink(type = "message")
          ...
          save(out, file = file.path(dir, "data", paste0(gsub(" ", "_", spec), "_cl_alt3_quadr_stronger_lasso_GoF.RData")))
