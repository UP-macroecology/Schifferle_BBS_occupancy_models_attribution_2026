...
# quadratic effects of bioclim covariates, lasso prior
...
sink("output_landuse_quadr_lasso_House_Finch_GoF.txt") # write console output here
...
model_file <- "dynoccmod_lu_quadr_lasso_GoF.txt"
...
## Declare priors ------------------------------------------------------------------------------
  ...
  # Detection Priors
  p ~ dunif(0.0001,0.9999) # detection probability: any value between 0 and 1
  ...
  # Iterate over sites:
	for (s in 1:nsites){
		...
		logit(psi1[s]) <- alpha_psi + 
		                  beta_psi[1]*sum_annual_crops_3yrs[s] + beta_psi[2]*c4per_3yrs[s] + beta_psi[3]*c3per_3yrs[s] +
		                  beta_psi[4]*primn_3yrs[s] + beta_psi[5]*secdn_3yrs[s] + beta_psi[6]*pastr_3yrs[s] +
		                  beta_psi[7]*urban_3yrs[s] +
		                  beta_psi[8]*pow(sum_annual_crops_3yrs[s],2) + beta_psi[9]*pow(c4per_3yrs[s],2) +
		                  beta_psi[10]*pow(c3per_3yrs[s],2) + beta_psi[11]*pow(primn_3yrs[s],2) +
		                  beta_psi[12]*pow(secdn_3yrs[s],2) + beta_psi[13]*pow(pastr_3yrs[s],2) +
		                  beta_psi[14]*pow(urban_3yrs[s],2)
    ...
    # State transitions
      ...
      # Set formula for Epsilon (Extinction)
			logit(eps[s, t-1]) <- alpha_eps + beta_eps[1]*sum_annual_crops[s,t] +
			                      beta_eps[2]*c4per[s,t] + beta_eps[3]*c3per[s,t] + beta_eps[4]*primn[s,t] +
			                      beta_eps[5]*secdn[s,t] + beta_eps[6]*pastr[s,t] + beta_eps[7]*urban[s,t] +
		                        beta_eps[8]*pow(sum_annual_crops[s,t],2) + beta_eps[9]*pow(c4per[s,t],2) +
		                        beta_eps[10]*pow(c3per[s,t],2) + beta_eps[11]*pow(primn[s,t],2) +
		                        beta_eps[12]*pow(secdn[s,t],2) + beta_eps[13]*pow(pastr[s,t],2) +
		                        beta_eps[14]*pow(urban[s,t],2)
			
			# Set formula for Gamma (Colonisation)
			logit(gamma[s, t-1]) <- alpha_gamma + beta_gamma[1]*sum_annual_crops[s,t] +
			                        beta_gamma[2]*c4per[s,t] + beta_gamma[3]*c3per[s,t] + beta_gamma[4]*primn[s,t] +
			                        beta_gamma[5]*secdn[s,t] + beta_gamma[6]*pastr[s,t] + beta_gamma[7]*urban[s,t] +
		                          beta_gamma[8]*pow(sum_annual_crops[s,t],2) + beta_gamma[9]*pow(c4per[s,t],2) +
		                          beta_gamma[10]*pow(c3per[s,t],2) + beta_gamma[11]*pow(primn[s,t],2) +
		                          beta_gamma[12]*pow(secdn[s,t],2) + beta_gamma[13]*pow(pastr[s,t],2) +
		                          beta_gamma[14]*pow(urban[s,t],2)
...
bdata <- list(y = y_array, 
              ...
              c4per_3yrs = occ_dt_spec$c4per_3yrs[seq(1, nrow(occ_dt_spec), nyears)],
              c3per_3yrs = occ_dt_spec$c3per_3yrs[seq(1, nrow(occ_dt_spec), nyears)],
              ...
              c4per = matrix(occ_dt_spec$c4per, nrow = nsites, ncol = nyears, byrow = TRUE),
              c3per = matrix(occ_dt_spec$c3per, nrow = nsites, ncol = nyears, byrow = TRUE),
...
save(out, file = file.path(cluster_import, "data", paste0(gsub(" ", "_", spec), "_lu_quadr_lasso_GoF.RData"))) # 2 GB
