...
# quadratic effects of bioclim covariates, alternative set 2 (manual decision), lasso prior, p intercept only
...
model_file <- "dynoccmod_cl_alt2_quadr_lasso_GoF.txt"
...
## Declare priors ------------------------------------------------------------------------------
  ...
  # Iterate over sites:
	for (s in 1:nsites){
		...
		logit(psi1[s]) <- alpha_psi + beta_psi[1]*bio1_3yrs[s] + beta_psi[2]*bio2_3yrs[s] + beta_psi[3]*bio3_3yrs[s] +
		                  beta_psi[4]*bio10_3yrs[s] + beta_psi[5]*bio12_3yrs[s] + beta_psi[6]*bio16_3yrs[s] +
		                  beta_psi[7]*pow(bio1_3yrs[s],2) + beta_psi[8]*pow(bio2_3yrs[s],2) +
		                  beta_psi[9]*pow(bio3_3yrs[s],2) + beta_psi[10]*pow(bio10_3yrs[s],2) +
		                  beta_psi[11]*pow(bio12_3yrs[s],2) + beta_psi[12]*pow(bio16_3yrs[s],2)
    ...
    # State transitions
      ...
      # Set formula for Epsilon (Extinction)
			logit(eps[s, t-1]) <- alpha_eps + beta_eps[1]*bio1[s,t] + beta_eps[2]*bio2[s,t] + 
			                      beta_eps[3]*bio3[s,t] + beta_eps[4]*bio10[s,t] + beta_eps[5]*bio12[s,t] +
			                      beta_eps[6]*bio16[s,t] + 
		                        beta_eps[7]*pow(bio1[s,t],2) + beta_eps[8]*pow(bio2[s,t],2) +
		                        beta_eps[9]*pow(bio3[s,t],2) + beta_eps[10]*pow(bio10[s,t],2) +
		                        beta_eps[11]*pow(bio12[s,t],2) + beta_eps[12]*pow(bio16[s,t],2)
			
			# Set formula for Gamma (Colonisation)
			logit(gamma[s, t-1]) <- alpha_gamma + beta_gamma[1]*bio1[s,t] + beta_gamma[2]*bio2[s,t] + 
			                        beta_gamma[3]*bio3[s,t] + beta_gamma[4]*bio10[s,t] + beta_gamma[5]*bio12[s,t] +
			                        beta_gamma[6]*bio16[s,t] +
		                          beta_gamma[7]*pow(bio1[s,t],2) + beta_gamma[8]*pow(bio2[s,t],2) +
		                          beta_gamma[9]*pow(bio3[s,t],2) + beta_gamma[10]*pow(bio10[s,t],2) +
		                          beta_gamma[11]*pow(bio12[s,t],2) + beta_gamma[12]*pow(bio16[s,t],2)
...
testspecs <- c("Eastern Kingbird", 
               "Yellow Warbler",
               "American Goldfinch")

# register cores for parallel computation:
ncores <- 9 # n species * 3 chains? 
...
foreach(spec = testspecs,
        ...
        .verbose = TRUE) %dopar% {
          ...
          bdata <- list(y = y_array, 
                        ...
                        bio12_3yrs = occ_dt_spec$bio12_3yrs[seq(1, nrow(occ_dt_spec), nyears)],
                        ...
                        bio12 = matrix(occ_dt_spec$bio12, nrow = nsites, ncol = nyears, byrow = TRUE),
          ...
          sink(paste0("out_cl_alt2_quadr_lasso_", gsub(" ", "_", spec),"_GoF.txt")) # write console output here
          sink(type = "message")
          ...
          save(out, file = file.path(dir, "data", paste0(gsub(" ", "_", spec), "_cl_alt2_quadr_lasso_GoF.RData")))
