...
# quadratic effects of bioclim covariates, alternative set, lasso prior, p intercept only
...
spec <- "American Kestrel"#"House Finch" # Eastern Meadowlark"
...
route_sel_env_dt_scaled <- route_sel_env_dt_final %>% 
  mutate(across(bio14:primf_3yrs, ~ (scale(.)) %>% as.vector()))
...
model_file <- "dynoccmod_cl_alt1_quadr_lasso_GoF.txt"
...
## Declare priors ------------------------------------------------------------------------------
  ...
  # Iterate over sites:
	for (s in 1:nsites){
		...
		logit(psi1[s]) <- alpha_psi + beta_psi[1]*bio2_3yrs[s] + beta_psi[2]*bio3_3yrs[s] + beta_psi[3]*bio10_3yrs[s] +
		                  beta_psi[4]*bio11_3yrs[s] + beta_psi[5]*bio16_3yrs[s] + beta_psi[6]*bio17_3yrs[s] +
		                  beta_psi[7]*pow(bio2_3yrs[s],2) + beta_psi[8]*pow(bio3_3yrs[s],2) +
		                  beta_psi[9]*pow(bio10_3yrs[s],2) + beta_psi[10]*pow(bio11_3yrs[s],2) +
		                  beta_psi[11]*pow(bio16_3yrs[s],2) + beta_psi[12]*pow(bio17_3yrs[s],2)
    ...
    # State transitions
      ...
      # Set formula for Epsilon (Extinction)
			logit(eps[s, t-1]) <- alpha_eps + beta_eps[1]*bio2[s,t] + beta_eps[2]*bio3[s,t] + 
			                      beta_eps[3]*bio10[s,t] + beta_eps[4]*bio11[s,t] + beta_eps[5]*bio16[s,t] +
			                      beta_eps[6]*bio17[s,t] + 
		                        beta_eps[7]*pow(bio2[s,t],2) + beta_eps[8]*pow(bio3[s,t],2) +
		                        beta_eps[9]*pow(bio10[s,t],2) + beta_eps[10]*pow(bio11[s,t],2) +
		                        beta_eps[11]*pow(bio16[s,t],2) + beta_eps[12]*pow(bio17[s,t],2)
			
			# Set formula for Gamma (Colonisation)
			logit(gamma[s, t-1]) <- alpha_gamma + beta_gamma[1]*bio2[s,t] + beta_gamma[2]*bio3[s,t] + 
			                        beta_gamma[3]*bio10[s,t] + beta_gamma[4]*bio11[s,t] + beta_gamma[5]*bio16[s,t] +
			                        beta_gamma[6]*bio17[s,t] +
		                          beta_gamma[7]*pow(bio2[s,t],2) + beta_gamma[8]*pow(bio3[s,t],2) +
		                          beta_gamma[9]*pow(bio10[s,t],2) + beta_gamma[10]*pow(bio11[s,t],2) +
		                          beta_gamma[11]*pow(bio16[s,t],2) + beta_gamma[12]*pow(bio17[s,t],2)
...
bdata <- list(y = y_array, 
              ...
              bio10_3yrs = occ_dt_spec$bio10_3yrs[seq(1, nrow(occ_dt_spec), nyears)],
              bio11_3yrs = occ_dt_spec$bio11_3yrs[seq(1, nrow(occ_dt_spec), nyears)],
              bio16_3yrs = occ_dt_spec$bio16_3yrs[seq(1, nrow(occ_dt_spec), nyears)],
              bio17_3yrs = occ_dt_spec$bio17_3yrs[seq(1, nrow(occ_dt_spec), nyears)],
              ...
              bio10 = matrix(occ_dt_spec$bio10, nrow = nsites, ncol = nyears, byrow = TRUE),
              bio11 = matrix(occ_dt_spec$bio11, nrow = nsites, ncol = nyears, byrow = TRUE),
              bio16 = matrix(occ_dt_spec$bio16, nrow = nsites, ncol = nyears, byrow = TRUE),
              bio17 = matrix(occ_dt_spec$bio17, nrow = nsites, ncol = nyears, byrow = TRUE),
              nsites = nsites, 
              nyears = nyears,
              nsurveys = nsurveys,
...
sink(paste0("out_cl_alt1_quadr_lasso_", gsub(" ", "_", spec),"_GoF.txt")) # write console output here
...
out <- autojags(data = bdata, inits = inits, parameters.to.save = params, 
                ...
                #save.all.iter = TRUE,
                Rhat.limit = 1.1,
                max.iter = 75000) # Rushing: 50000, Mingjian: 20000

sink(type = "message") # write console output to file

save(out, file = file.path(dir, "data", paste0(gsub(" ", "_", spec), "_cl_alt1_quadr_lasso_GoF.RData"))) # 2 GB

unlink(paste0("out_cl_alt1_quadr_lasso_", gsub(" ", "_", spec),"_GoF.txt"))
