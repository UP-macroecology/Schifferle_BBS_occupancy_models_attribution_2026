# calculate evaluation measures based on cross validation for 
# models each refitted with one detrended variable:
...
model_output <- list.files(path = file.path(results_dir), pattern = "test_preds_")

spec <- unlist(strsplit(model_output[1], split = "_"))[3]
print(spec)

vars <- unique(gsub(pattern = paste0("(test_preds_", spec, "_)|(_CV_fold.*.RData)"), 
             replacement = "", 
             x = model_output)) # extracts variable name from file name
...
for(var in vars){
  ...
  for(fold in 1:5){
    ...
    load(file = file.path(results_dir, paste0("test_preds_", spec, "_", var, "_CV_fold", fold, ".RData")))
  ...
  save(CV_eval, file = file.path(dir, "results", "attribution", paste0("CV_eval_", spec, "_", var, ".RData")))
...
CV_eval_summary <- data.frame("species" = spec,
                              "var" = c("org", vars),
                              "y_spattemp_C" = NA,
                              "occ_spattemp_C" = NA,
                              "y_spattemp_auc" = NA,
...
for(var in vars){
  ...
  skip_to_next <- FALSE
  tryCatch(print(load(file = file.path(dir, "results", "attribution", paste0("CV_eval_", spec, "_", var, ".RData")))),
           error = function(e) { skip_to_next <<- TRUE})
  if(skip_to_next) { next }
  
  CV_eval_summary$y_spattemp_C[which(CV_eval_summary$species == spec & CV_eval_summary$var == var)] <- CV_eval$C_spattemp_y["C Index"]
  CV_eval_summary$occ_spattemp_C[which(CV_eval_summary$species == spec & CV_eval_summary$var == var)] <- CV_eval$C_spattemp_occ["C Index"]
  
  CV_eval_summary$y_spattemp_auc[which(CV_eval_summary$species == spec & CV_eval_summary$var == var)] <- CV_eval$auc_spattemp_y
  CV_eval_summary$occ_spattemp_auc[which(CV_eval_summary$species == spec & CV_eval_summary$var == var)] <- CV_eval$auc_spattemp_occ
  
  CV_eval_summary$y_spatial_C_mean[which(CV_eval_summary$species == spec & CV_eval_summary$var == var)] <- mean(CV_eval$C_spat_y[, "C Index"], na.rm = TRUE)
  CV_eval_summary$occ_spatial_C_mean[which(CV_eval_summary$species == spec & CV_eval_summary$var == var)] <- mean(CV_eval$C_spat_occ[, "C Index"], na.rm = TRUE)  
  
  CV_eval_summary$y_spatial_auc_mean[which(CV_eval_summary$species == spec & CV_eval_summary$var == var)] <- mean(CV_eval$auc_spat_y[ ,"auc"], na.rm = TRUE)
  CV_eval_summary$occ_spatial_auc_mean[which(CV_eval_summary$species == spec & CV_eval_summary$var == var)] <- mean(CV_eval$auc_spat_occ[ ,"auc"], na.rm = TRUE)
  
  CV_eval_summary$y_temp_C[which(CV_eval_summary$species == spec & CV_eval_summary$var == var)] <- CV_eval$C_temp_y["C Index"]
  CV_eval_summary$occ_temp_C[which(CV_eval_summary$species == spec & CV_eval_summary$var == var)] <- CV_eval$C_temp_occ["C Index"]
  
}

# add metrics for model with original data:
CV_eval_summary_org <- read.csv(file = file.path(dir, "results", "CV_cluster", "CV_eval2", "CV_eval_summary.csv"))
CV_eval_summary_org_spec <- CV_eval_summary_org %>% 
  filter(species == spec)

CV_eval_summary[which(CV_eval_summary$species == spec & CV_eval_summary$var == "org"), 3:ncol(CV_eval_summary)] <-
  CV_eval_summary_org_spec[,2:ncol(CV_eval_summary_org_spec)]


write.csv(CV_eval_summary, file = file.path(dir, "results", "attribution", "CV_eval_summary.csv"), row.names = FALSE)
