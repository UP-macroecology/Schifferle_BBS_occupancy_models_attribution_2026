# attribution
# 5 fold spatially blocked CV to assess predictive ability of dynamic occupancy models:
...
ncores <- 60 # 5 fold * 4 chains * vars
cl <- makeCluster(ncores, setup_timeout = 0.5)
registerDoParallel(cl)


foreach(var = c("bio1", "bio3", "pr_summer"), #c("bio1", "bio3", "pr_summer", "pr_winter", "sum_annual_crops", "secdf", "urban"),
        .packages = c("dplyr", "collapse", "flocker", "cmdstanr", "brms", "sf"), # xx
        .errorhandling = "pass", #"remove",
        .verbose = TRUE) %:% 
  
  foreach(fold = 1:5,
          .packages = c("dplyr", "collapse", "flocker", "cmdstanr", "brms"),
          .errorhandling = "pass", #"remove",
          .verbose = TRUE) %dopar% {
            
            prog_log_file <- file(file.path("logfiles", "attribution", "CV", paste0(spec, "_", var, "_progress_fold", fold, ".txt")), open = "wt") # write console output here
            ...
            for (t in 1:nyears){
              env_cov[[t]] <- env_dt_spec_detr_scaled[which(env_dt_spec_detr_scaled$Year == years[t] & env_dt_spec_detr_scaled$RTENO %in% occ_dt_spec_subset$RTENO), 
            ...
            save(out, file = file.path(dir, "results", "attribution", "CV", paste0("out_", spec, "_", var, "_CV_fold", fold, ".RData")))
            ...
            # data for test folds:
            test_env_data <- env_dt_spec_detr_scaled %>%
            ...
            for (t in 1:nyears){
              env_cov_test[[t]] <- test_env_data[which(test_env_data$Year == years[t]), 
                                                           c("bio1", "bio2", "bio3", "bio7", "bio14", "bio15", 
                                                             "pr_spring", "pr_summer","pr_autumn", "pr_winter",
                                                             "bio1_3yrs", "bio2_3yrs", "bio3_3yrs", "bio7_3yrs", "bio14_3yrs", "bio15_3yrs",
                                                             "pr_spring_3yrs", "pr_summer_3yrs", "pr_autumn_3yrs", "pr_winter_3yrs", 
            ...
            save(res_list, file = file.path(dir, "results", "attribution", "CV", paste0("test_preds_", spec, "_", var, "_CV_fold", fold, ".RData")))
